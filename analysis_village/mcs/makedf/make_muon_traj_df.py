...
from analysis_village.mcs.modules.TrajectoryMCSFitter import *
...
def select_topology(df, nprong=2, max_len_cut=25, min_len_cut=25, contained=True):
    ...
    if nprong == 2:
        nprong_df['is_len'] = nprong_df.pfp.trk.len > 21.
...
def get_coszy(df):
    diry = df.pfp.trk.dir.y
    dirz = df.pfp.trk.dir.z
    coszy = np.abs(diry / np.hypot(diry, dirz))
...
def make_muon_hit_df(f):
    ...
    muon_candidate_true_p = np.sqrt(muon_candidate.pfp.trk.truth.p.genp.x**2 + muon_candidate.pfp.trk.truth.p.genp.y**2 + muon_candidate.pfp.trk.truth.p.genp.z**2)
    ...
    muon_hits['reco_len'] = muon_candidate.pfp.trk.len
    muon_hits['gen_p'] = muon_candidate_true_p
...
def make_muon_traj_df(f):
    ...
    for group_keys, group_df in grouped:
        #print(group_df)
        this_traj = Trajectory(group_df)
        prep_sp = fitMcsPrepare(this_traj)

        if prep_sp is None:
            # Skip the rest of this loop and go to the next group
            continue
        
        #print(prep_sp)
        #print(group_df.gen_p)
        prep_sp["gen_p"] = group_df.gen_p.iloc[0]

        prep_sp["entry"] = group_keys[0]
        prep_sp["rec.slc..index"] = group_keys[1]
        prep_sp["rec.slc.reco.pfp..index"] = group_keys[2]

        prep_sp = prep_sp.set_index([
            "entry", 
            "rec.slc..index", 
    ...
    if len(this_sp_dfs) > 0:
        final_mcs_df = pd.concat(this_sp_dfs)
    else:
        final_mcs_df = pd.DataFrame()
