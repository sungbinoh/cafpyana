...
def make_pandora_no_cuts_df(f):
    print("Starting: make_pandora_no_cuts_df")
...
def make_PAC_nuslimwgtdf(f):
    print("Starting: make_PAC_nuslimwgtdf")
...
def make_PAC_nudf(f, is_slc=False):
    print("Starting: make_PAC_nudf")
    ...
    this_nudf = pd.DataFrame({
        'nu_E': true_nu_E.astype('float32'),
        'del_p': true_del_p.astype('float32'),
        'genie_mode': genie_mode.astype('int'), 
        'is_sig': is_sig.astype('bool'), 
        'is_contained': is_contained.astype('bool'),
        'is_nc': is_nc.astype('bool'), 
        'is_other_numucc': is_other_numucc.astype('bool'), 
        'is_fv': is_fv.astype('bool'), 
        'pos_x' : nudf.position.x.astype('float32'),
        'pos_y' : nudf.position.y.astype('float32'),
        'pos_z' : nudf.position.z.astype('float32'),
        'pdg': pdg.astype('int'),
        'nmu': nmu.astype('int'),
        'nn': nn.astype('int'),
        'np': np.astype('int'),
        'npi': npi.astype('int'),
        'npi0': npi0.astype('int') 
    })
