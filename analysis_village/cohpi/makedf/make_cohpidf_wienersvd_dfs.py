...
def safe_filter(df, mask, name):
    """Apply mask safely and preserve columns if result is empty."""
    mask = mask.fillna(False) if mask.dtype == bool else mask
    prev_len = len(df)
    newdf = df[mask]
    new_len = len(newdf)
    #print(f"{name:25s} -> {new_len:5d} rows left (filtered {prev_len - new_len})")

    if new_len == 0:
        newdf = df.iloc[0:0].copy()
    return newdf
...
def make_cohpidf_slc(f):
    ...
    #### (9) last moment evt selections
    slcdf = safe_filter(slcdf, slcdf.is_not_clear_cosmic & slcdf.is_fv, "is_not_clear_cosmic & is_fv")
    #print("is_not_clear_cosmic & is_fv: len(slcdf) = %d" %len(slcdf))
    
    hdrdf = make_mchdrdf(f)
    ismc = hdrdf.ismc.iloc[0]
    PE_cut = 2000.
    if ismc:
        slcdf['total_pe'] = slcdf.total_pe * 0.66
    slcdf = safe_filter(slcdf, slcdf.total_pe > PE_cut, f"total_pe > {PE_cut}")
    #print("total_pe > {PE_cut}: len(slcdf) = %d" %len(slcdf))

    slcdf = safe_filter(slcdf, slcdf.n_prong == 2, "n_prong == 2")
    #print("n_prong == 2: len(slcdf) = %d" %len(slcdf))
    slcdf = safe_filter(slcdf, slcdf.n_trk == 2, "n_trk == 2")
    slcdf = safe_filter(slcdf, slcdf.n_proton == 0, "n_proton == 0")
    slcdf = safe_filter(slcdf, slcdf.n_shower == 0, "n_shower == 0")
    #print("n_shower == 2: len(slcdf) = %d" %len(slcdf))

    slcdf = safe_filter(slcdf, slcdf.long_dirz_pass.fillna(False), "long_dirz_pass")
    slcdf = safe_filter(slcdf, slcdf.short_dirz_pass.fillna(False), "short_dirz_pass")
    #print("dirz pass: len(slcdf) = %d" %len(slcdf))

    slcdf = safe_filter(slcdf, slcdf.opening_angle > 0.5, "opening_angle > 0.5")
    #print("opening angle: len(slcdf) = %d" %len(slcdf))
    
    slcdf = safe_filter(slcdf, slcdf.long_trk_pid < 0.1, "long_trk_pid < 0.1")
    slcdf = safe_filter(slcdf, slcdf.short_trk_pid < 0.1, "short_trk_pid < 0.1")
    #print("further trk pid: len(slcdf) = %d" %len(slcdf))
    
    slcdf = safe_filter(
        slcdf, (slcdf.long_trk_len > 20.) & (slcdf.long_trk_len < 500.), 
        "20 < long_trk_len < 500"
    )
    slcdf = safe_filter(
        slcdf, (slcdf.short_trk_len > 10.) & (slcdf.short_trk_len < 200.), 
        "10 < short_trk_len < 200"
    )
    

    
    slcdf = safe_filter(slcdf, np.abs(slcdf.long_diry) < 0.5, "|long_diry| < 0.5")
    #print("|long_diry| < 0.5: len(slcdf) = %d" %len(slcdf))

    slcdf.sum_integ_1cm = slcdf.sum_integ_1cm.fillna(-1)
    slcdf.sum_integ_2cm = slcdf.sum_integ_2cm.fillna(-2)
    slcdf.sum_integ_3cm = slcdf.sum_integ_3cm.fillna(-3)
    slcdf.sum_integ_4cm = slcdf.sum_integ_4cm.fillna(-4)
    slcdf['sum_integ_ratio_3over4sub'] = slcdf.sum_integ_3cm / (slcdf.sum_integ_4cm - slcdf.sum_integ_3cm)
    slcdf = safe_filter(slcdf, slcdf.sum_integ_1cm < 4000., "sum_integ_1cm < 4000")
    slcdf = safe_filter(slcdf, slcdf.sum_integ_2cm < 6000., "sum_integ_2cm < 6000")
    slcdf = safe_filter(slcdf, slcdf.sum_integ_3cm < 8000., "sum_integ_3cm < 8000")
    slcdf = safe_filter(slcdf, slcdf.sum_integ_4cm < 10000., "sum_integ_4cm < 10000")
    slcdf = safe_filter(slcdf, slcdf.sum_integ_ratio_3over4sub < 6., "sum_integ_ratio_3over4sub < 6")
...
def make_cohpi_nudf(f):
    ...
    ## to study each genie weight
    geniewgtdf = geniesyst.geniesyst(f, nudf.ind, multisim_nuniv=100, slim=True, systematics=None)
    ## to study only CCCOH knob
    #geniewgtdf = geniesyst.geniesyst(f, nudf.ind, multisim_nuniv=100, slim=False, systematics=["GENIEReWeight_SBN_v1_multisigma_NormCCCOH"])
    
    #geniewgtdf = geniesyst.geniesyst(f, nudf.ind, multisim_nuniv=100, slim=True, systematics=None)
    bnbwgtdf = bnbsyst.bnbsyst(f, nudf.ind, multisim_nuniv=100, slim=True)
    #g4wgtdf = g4syst.g4syst(f, nudf.ind) # not in v10_06_00_05 flat.cafs

    this_nudf = multicol_concat(this_nudf, geniewgtdf)
    this_nudf = multicol_concat(this_nudf, bnbwgtdf)
