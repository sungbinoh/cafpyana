...
def match_across_detvar(files):
    ...
    for f in files:
        ...
        recomatchdf = recomatchdf[['run', 'subrun', 'evt', 'tmatch_idx']]
    ...
    filt_mcdataframes, frac = filter_n_common_events(unfilt_mcdataframes, keys=["run", "subrun", "evt", "nu_E"])

    for filt_mc, reco in zip(filt_mcdataframes, unfilt_recodataframes):

        filt_mc.drop(columns=[("run", ""), ("subrun", ""), ("evt", ""), ("nu_E", "")], inplace=True)


        df = ph.multicol_merge(reco.reset_index(), filt_mc.reset_index(),
                               left_on=[("__ntuple", ""), ("entry", ""), ("tmatch_idx", "")],
                               right_on=[("__ntuple", ""), ("entry", ""), ("rec.mc.nu..index", "")],
                               how="left").set_index(['__ntuple', 'entry', 'rec.slc..index'])

        pot = df.pot.to_numpy()
        pot_nz = pot[pot != 0]

        print("final pot: \n", np.sort(pot_nz))
        #print("final pot: ", df.sort_index().pot)
